<?
$i=0;
$cantDocs=admin::getDbValue("select count(*) from mdl_contents_docs where mcd_con_uid='".$con_uid."' and mcd_delete=0 and mcd_status='ACTIVE'");
...
<table width="98%" border="0" cellpadding="5" cellspacing="5" class="box">			
	   ...
	   <div class="items">
            ...
            <div class="listOpt" id="child">
              <!-- <ul id="child">-->
               <?php
	  			$sql3 =  "select * from mdl_contents_docs where mcd_con_uid='".$con_uid."' and mcd_delete=0 and mcd_status='ACTIVE'";
				//krumo($sql);
				$db3->query($sql3);
			   ...
			   ?>
					...
					<table width="100%" border="0" cellpadding="5" cellspacing="5">
                  	...
                  	<tr>
            			<td colspan="2">
							<? 
                            $imgSavedroot2 = PATH_ROOT."/docs/content/".$content3["mcd_ruta"];
                            $imgSaveddomain2 = PATH_ROOT."/docs/content/".$content3["mcd_ruta"];
                            //echo $imgSaveddomain2;die;
                            if (file_exists($imgSavedroot2) && $content3["mcd_ruta"]!="")
                                { 
                                $extension = admin::getExtension($content3["mcd_ruta"]);
                            //echo $extension;die;
                                $imgextension = admin::getExtensionImage($extension);
                            //	echo $imgextension;die;
                            ?>
                            <div id="document_edit_<?=$content3["mcd_uid"]?>">
                            <div id="changeFile">
                            ...
                            <tr>
                                ...
                                <span class="nameFile"><?=substr($content3["mcd_ruta"],0,20);?>...</span>
                            <br />
                            <!--<a href="javascript:changeUploadFile('on',<?=$i?>)" class="small2">
                            <?=admin::labels('change');?>
                                </a> <span class="pipe">|</span>--> <a href="#" onclick="removeDoc(<?=$content3["mcd_uid"]?>);return false;" title="<?=admin::labels('del')?>" class="small3"><?=admin::labels('del')?></a>				</td>
                            </tr>
                            <tr>
                                <td  valign="top"><div id="div_adjunt_file_change_<?=$i?>" style="display:none">
                                <table border="0" cellspacing="0" cellpadding="0" width="100%"><tr><td>
                                <input type="file" name="edit_adjunt_<?=$content3["mcd_uid"]?>" id="edit_adjunt_<?=$content3["mcd_uid"]?>" size="13" style="font-size:11px;" class="input5"></td>
                                <td> <a href="#" onclick="changeUploadFile(<?=$i?>); return false;"><img border="0" src="lib/close.gif" align="top"/></a>				</td>
                                </tr>
                                </table>
                                </div>				</td>
                            </tr>
                            ...
                            <div id="document_add_<?=$content3["mcd_uid"]?>" style="display:none;"></div>
                            <? } 
                            else
                                { ?>
