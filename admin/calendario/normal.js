...
var gWeekDay=["Do","Lu","Ma","Mi","Ju","Vi","Sa"];	// Dias en español
var gBegin=[1950,1,1];	// calendar date range begin from [Year,Month,Date]. Using gToday here will make it start from today.
var gEnd=[2030,12,31];	// calendar date range end at [Year,Month,Date]
var gsOutOfRange="Lo siento, no se encuentra en el campo designado!";	// out-of-date-range error message. If set to "", no alerts will popup on such error.
...
var gcCalBG="#3f4e5c";	// the background color of the outer calendar panel.
...
var gsBottom="<A href='javascript:void(0)' class='BottomAnchor' onclick='if(this.blur)this.blur();if(!fSetDate(gToday[0],gToday[1],gToday[2]))alert(\"You cannot select today!\");return false;' onmouseover='return true;' >Hoy : "+gMonths[gToday[1]-1]+" "+gToday[2]+", "+gToday[0]+"</A>";	// the content of the bottom section.
...
var gcToggle="#ff6b05";	// the highlight color for the focused cell
