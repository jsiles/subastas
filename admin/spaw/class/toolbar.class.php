...
//require_once(str_replace('\\\\','/',dirname(__FILE__)).'/parsecss.php');
/**
...
class SpawToolbar
{
  ...
  { 
          ...
          switch(strtolower(get_class($obj)))
          {
            ...
            case "spawtbdropdown":
            {
              ...
              if (is_array($obj->data))
              {
                ...
                {  
                  $style = $tpl->load_file ('main',$key);
                  if ($style=='') $style="none1";
                  $html_res .= '<option value="'.$key.'" style="'.$style.'">'.$value.'</option>'; 
                }
                $html_res .= '</select>';
              }
            }
   ...
   function load_file($sName, $style)
  {
    $nName = "";
    $template_path = "spaw/wysiwyg.css";
    if (file_exists($template_path))
    {
      $this->DBlocks[$sName] = join('',file($template_path));
      $bStyle =  $this->getBlock($sName,$style);
      return $bStyle;
    }
  }
  function getBlock($sTemplateName, $sName)
  {
    $sTemplate = $this->DBlocks[$sTemplateName];
    $alpha = strlen($sName) + 18;
