...
/*# AVOID COLLISIONS #*/
 ...
 $.extend($.MultiFile, {
  options: {
   ...
   STRING: {
    remove:'remove',
    denied:'Usted no puede seleccionar el archivo de extencion $ext .\nIntenta de nuevo...',
    selected:'Archivo seleccionado: $file',
    duplicate:'Este archivo ya se encuentra seleccionado:\n$file'
   }
  }
 });
    ...
    return $(this).each(function(group_count){
     if(this._MultiFile) return; this._MultiFile = true;
       ...
       MF.addToList = function( slave ){
        ...
        var
         r = $('<div style="padding: 2px 10px; margin-left: 100px;"></div>'),
         v = String(slave.value || ''/*.attr('value)*/),
         a = $('<span class="file" title="'+MF.STRING.selected.replace('$file', v)+'">'+v.match(/[^\/\\]+$/gi)[0]+'</span>'),
         b = $('<a href="#'+MF.wrapID+'" class="link2 color3">'+MF.STRING.remove+'</a>');
